...
class MediaSearchScreen extends StatefulWidget with GetItStatefulWidgetMixin {
  MediaSearchScreen({Key? key, required this.type}) : super(key: key);
...
class _MediaSearchScreenState extends State<MediaSearchScreen> with GetItStateMixin,SingleTickerProviderStateMixin {
  ...
  late final SearchVizController vizController = SearchVizController(
    _searchResults,
    minYear: resumeLogic.timelineStartYear,
    maxYear: resumeLogic.timelineEndYear,
  );

  late double _startYear = 100 * 1.0, _endYear = 2030 * 1.0;
  ...
  void _search() async {
    _searchResults = await resumeLogic.getResumePresentationData();
    _searchSuggestions = await resumeLogic.getResumePresentationSuggestions();
    ...
    if (!_query.isEmpty) {
      ...
      _searchResults = _searchResults.where((o) => o.title.contains(q) || o.keywords.contains(q)).toList();
    }
  ...
  Widget build(BuildContext context) {
    ...
    Widget content = GestureDetector(
      ...
      child: Column(
        ...
        children: [
          SimpleHeader($strings.artifactsSearchTitleBrowse, subtitle: "作品一覧"),
          Gap($styles.insets.xs),
          Padding(
            padding: EdgeInsets.symmetric(horizontal: $styles.insets.sm),
            child: _SearchInput(onSubmit: _handleSearchSubmitted, searchSuggestions:_searchSuggestions),
          ),
          Expanded(
